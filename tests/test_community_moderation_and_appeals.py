...
class TestCommunityModerationAndAppeals(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_exact_4_day_restriction_expiry_timestamp_sent_to_telegram(self):
        ...
        self.assertIn("🔒 Restricted until:", group_text)
        # Check that the date format is correct (should be "19 Sep 2026, 04:48 AM" format)
        self.assertIn(until_date.strftime("%d %b %Y, %I:%M %p").lower(), group_text.lower())
        ...
        self.assertIn("🔒 Restricted until:", dm_text)
        # Check that the date format is correct (should be "19 Sep 2026, 04:51 AM" format)
        self.assertIn(until_date.strftime("%d %b %Y, %I:%M %p").lower(), dm_text.lower())
